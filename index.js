...
function controls(){
	...
	if(char.state == -1&&keysDown[65] != true&&char.jumpState == 1&&char.yv < -4){
		char.yv = -4;
	}
	...
	if(char.state == -1&&char.jumpState != 2){
		if(!(keysDown[86]&&devMode)&&keysDown[39]){
			char.goingLeft = true;
			if(char.Gv<char.TOP){
				char.Gv += char.ACC*2;//*Math.cos(char.angle);
	...
	if(keysDown[65] == true&&char.pDropDash == true&&char.state == -1){
		char.dropCharge += 1;
	}
...
function physics(){
	//hit the beginning of the stage
	if(char.x < 15){
		char.x = 15;
		if(char.Gv < 0){
			char.Gv = -0.001;
	...
	if(char.state >= 0 && char.y != NaN){
		...
		if(char.state == 0){//if you are on the ground
			//sense the ground
			var backSense = senseVLine(char.x-9,char.y-20,36,c,char.layer);
			var frontSense = senseVLine(char.x+9,char.y-20,36,c,char.layer);
			...
			//sets the y position and angle if the ground is detected with both
			if(backSense[0] != false && frontSense[0] != false){
				...
				//char.angle = Math.abs(angle1-char.angle) < Math.PI/4?angle1:(Math.abs(backSense[2]-char.angle) < Math.PI/4?backSense[2]:Math.abs(frontSense[2]-char.angle) < Math.PI/4?frontSense[2]:char.angle)
				char.angle = angle1;
			}
			...
			//detect the ground
			var backSense = senseHLineR(char.x-20,char.y-9,36,c,char.layer);
			var frontSense = senseHLineR(char.x-20,char.y+9,36,c,char.layer);
			...
			var backSense = senseHLineL(char.x+20,char.y-9,36,c,char.layer);
			var frontSense = senseHLineL(char.x+20,char.y+9,36,c,char.layer);
			if(backSense[0] != false && frontSense[0] != false){
			...
			var backSense = senseVLineB(char.x-9,char.y+20,36,c,char.layer);
			var frontSense = senseVLineB(char.x+9,char.y+20,36,c,char.layer);
			//console.log("char.y b4: "+char.y+"backSense:"+backSense+"frontSense:"+frontSense);
			if(backSense[0] != false && frontSense[0] != false){
				//console.log("min value b4: "+Math.min(backSense[1],frontSense[1]))
				char.y = (isNaN((backSense[1]+frontSense[1])/2)?char.y:((backSense[1]+frontSense[1])/2));
				var angle1 = ((isNaN(backSense[2]*Math.PI/180)?char.angle:backSense[2]*Math.PI/180)+(isNaN(frontSense[2]*Math.PI/180)?char.angle:frontSense[2]*Math.PI/180))/2;
				char.angle = angle1;//(Math.abs(angle1-char.angle) < Math.PI/4)?angle1:(Math.abs(backSense[2]*Math.PI/180-char.angle) < Math.PI/4?backSense[2]*Math.PI/180:Math.abs(frontSense[2]*Math.PI/180-char.angle) < Math.PI/4?frontSense[2]*Math.PI/180:char.angle);
		...
		//console.log("char.y: "+char.y+" Char.yv: "+char.yv)
		if(char.rolling){
		...
		var backSense = senseVLine(char.x-7+rotX,char.y-15+rotY,36,c,char.layer);
		var frontSense = senseVLine(char.x+7+rotX,char.y-15+rotY,36,c,char.layer);
		...
		var backTopSense = senseVLineB(char.x-9+rotX,char.y-15+rotY,36,c,char.layer);
		var frontTopSense = senseVLineB(char.x+9+rotX,char.y-15+rotY,36,c,char.layer);
		if(LsideSense[1] == RsideSense[1]+1){//if we have some dumb problems
			char.yv -= char.GRV;
			char.y -= char.yv*1.5;
			...
			backSense = senseVLine(char.x-7+rotX,char.y-15+rotY,36,c,char.layer);
			frontSense = senseVLine(char.x+7+rotX,char.y-15+rotY,36,c,char.layer);
			backTopSense = senseVLineB(char.x-7+rotX,char.y-15+rotY,36,c,char.layer);
			frontTopSense = senseVLineB(char.x+7+rotX,char.y-15+rotY,36,c,char.layer);
		}
		...
		char.x += Math.abs(Math.sin((char.angle-char.pAngle)/2)*(h1))*Math.sin(char.angle);
		char.y += Math.abs(Math.sin((char.angle-char.pAngle)/2)*(h1))*Math.cos(char.angle);
	}
	//slide down a slope
	...
	for(var i = 0; i < level[0].length; i++){
		debug.addRect(char.x-15+15*Math.sin(char.angle),char.y-20-20*Math.cos(char.angle),30,40,"#00550011")
	...
	//char.yv = Math.floor(char.yv*10000)/10000;
	//char.xv = Math.floor(char.xv*10000)/10000;
	//console.log("char.y after vel: "+char.y)
	//hurt
	if(char.hurt&&char.rings <= 0&&char.invincible == 0){
		resetLevel();
		char.hurt = false;
	}
	if(char.hurt&&char.rings > 0&&char.invincible == 0){
		char.hurt = false;
		var angle = Math.PI/Math.min(char.rings,32);
		for(var i = 0; i < Math.min(char.rings,32); i++){
...
//draw Character
	...
	if(char.levitate==true&&char.GRV == 0&&char.state == -1){
...
var vScreenH = 224;
var vScreenW = 398;//320;

document.body.style.background = "black";
canvi.height = vScreenH*3;
canvi.width = vScreenW*3;
size(0)
...
function loop(){
	/*if(chunkBacklog.length > 0){
		...
		c.fillStyle = "black";
		c.fillText("Loading Chunks... "+chunkBacklog.length+" left",20,20);
		var img = chunkBacklog.shift();
		chunks[chunks.length] = autoChunk(img);
		char.currentAnim = anim.run;
		char.frameIndex += Math.abs(char.animSpeed);
		if(char.frameIndex>char.currentAnim.length-1){char.frameIndex = 0;}
		sonicCanvas.width = char.currentAnim[Math.floor(char.frameIndex)][2];
		sonicCanvas.height = char.currentAnim[Math.floor(char.frameIndex)][3];
		sc.imageSmoothingEnabled = false;
		sc.drawImage(sonicImage,-char.currentAnim[Math.floor(char.frameIndex)][0],-char.currentAnim[Math.floor(char.frameIndex)][1]);
		c.drawImage(sonicCanvas,canvi.width/2,canvi.height/2);
	}*/
	...
	if(continue1&&(keysDown[67]?slowmo1%4==0:true)){//allow to freeze for developer purposes if things get too out of hand.
		...
		//camera position
		//console.log("cam:"+(-level[1].length*128+vScreenW/6).toString()+" player:"+char.x.toString());
		...
		cam.tx = cam.x+debug.camX;//+Math.abs(char.xv*fpsFactor-char.xv);//lerp(cam.tx,cam.x,0.5);//
		cam.ty = cam.y+debug.camY;//+Math.abs(char.yv*fpsFactor-char.yv);//lerp(cam.ty,cam.y,0.5);//
		...
		if(devMode){
			...
			c.fillStyle = "black";
			c.fillText("Angle (deg):"+(char.angle*180/Math.PI).toString(),200,10);
			c.fillText("Wall state: "+(char.state).toString(),200,20);
			c.fillText("Hor. Velocity: "+(char.Gv).toString(),200,30);
			c.fillText("FPS factor: "+fpsFactor,200,40);
			c.fillText("FPS: "+(1000/(newMillis>lastMillis?newMillis-lastMillis:newMillis-(lastMillis-1000))).toString(),200,50);
		...
		if(touchControlsActive){
			...
			c.fillRect(30,400,200,200);
			c.fillRect(750,450,150,150);
			c.fillStyle = "#990000";
			if(keysDown[39]){
				c.fillRect(130,450,100,100);
			}
			if(keysDown[37]){
				c.fillRect(30,450,100,100);
			}
			if(keysDown[38]){
				c.fillRect(80,400,100,100);
			}
			if(keysDown[40]){
				c.fillRect(80,500,100,100);
			}
		...
		if(e.keyCode == 65&&char.pHoming == false&&keysDown[65] == false){
			...
			for(var i = 0; i < level[0].length; i++){
				if(level[0][i].targetable == true&&(level[0][i].x+level[0][i].w/2 > char.x) == char.goingLeft){
					//(level[0][i].x+level[0][i].w/2)-char.x;
					//(level[0][i].y+level[0][i].h/2)-char.y;
...
function updateTouch(touches){
	...
	/*var touch1 = {
		x:((e.touches[0].clientX-cRect.x)/cRect.width)*vScreenW*3,
		y:((e.touches[0].clientY-cRect.y)/cRect.height)*vScreenH*3
	};*/
	var touch1 = {
		x: 0,
		y: 0,
	...
	if(touch2.x > 750 && touch2.x < 900 && touch2.y > 450 && touch2.y < 600 && touch2.total > 0){//jump button
		if(keysDown[65] == false){
			if(char.rolling&&char.currentAnim == anim.spindash){
				sfx.src = "";
				sfx.src = sfxObj.spindash;
	...
	if(touch1.x > 30 && touch1.x < 230 && touch1.y > 400 && touch1.y < 600 && touch1.total > 0){//touchpad
		if(touch1.y > 450 && touch1.y < 550){
			if(touch1.x > 130){
				keysDown[39] = true;
				keysDown[37] = false;
			}
